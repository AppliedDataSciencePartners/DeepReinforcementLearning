...
class Game:
	...
	def step(self, action):
		next_state, value, done = self.gameState.takeAction(action) #the value of the next_state from the POV of the new playerTurn i.e. -1 if the previous player played a winning move
...
class GameState():
	...
	def checkForThreat(self):
		if np.count_nonzero(self.board) == 42:
			return 0

		for x,y,z,a in self.winners:
			if (self.board[x] + self.board[y] + self.board[z] + self.board[a] == 3 * -self.playerTurn):
				return 1
		return 0
